...
import com.mycompany.phone_store.service.AccountRoleService;
import com.mycompany.phone_store.service.AccountService;
import com.mycompany.phone_store.service.OrderService;
import com.mycompany.phone_store.service.VerifyEmailService;
...
public class AccountController {
    ...
    public String updateAccount(Model model, @ModelAttribute(value = "account") AccountEntity acc) {
        ...
        model.addAttribute("messageSuccess", "Successfully updated account ");
        model.addAttribute("account", account);
        return "account/profile";
    }
    ...
    public String updatePassword(Model model, 
            ...
            String hashpassword1 = HashPasswordUltil.hashPassword(newpassword2);
            String hashpassword2 = HashPasswordUltil.hashPassword(newpassword2);
        if (hashpassword1.equals(hashpassword2)) {
            AccountEntity account = AccountUltil.getAccount();
            
            if (account.getPassword().equals(hashpassword)) {
                account.setPassword(hashpassword1);
                accountService.saveAccount(account);
    ...
    public String registerAccount(Model model, @ModelAttribute(value = "account") AccountEntity account, @ModelAttribute(value = "password1") String password1, @ModelAttribute(value = "password2") String password2) {
        ...
        String hashpassword = HashPasswordUltil.hashPassword(password2);
        account.setPassword(hashpassword);
        List<AccountRoleEntity> accountRoleEntitys = new ArrayList<>();
        accountRoleEntitys.add(accountRoleService.getAccountRolesByRole(AccountRole.ROLE_USER));
        ...
        if (accountSaved != null && accountSaved.getId() > 0) {
            ...
            if (verifyCodeSaved != null & verifyCodeSaved.getId() > 0) {
                try {
                    ...
                    String htmlMsg = "<p>To activate your account, please verify your email address by clicking on the link below:</p><a href=\"http://localhost:8084/Phone_store/verify?email=" + accountSaved.getEmail() + "&code=" + verifyCode + "\">Verify now</a>";
