...
import com.mycompany.phone_store.entities.ShippingEntity;
import com.mycompany.phone_store.enums.OrderStatus;
import com.mycompany.phone_store.service.ColorService;
import com.mycompany.phone_store.service.OrderDetailService;
...
public class OderController {
    ...
    @RequestMapping("/order/{productId}/{colorId}")
    public String orderProduct(HttpSession session, @PathVariable(value = "productId") int productId, @PathVariable(value = "colorId") int colorId) {
        ...
        if (order == null) {
            ...
            for (OrderDetailEntity orderDetail : orderDetails) {
                if (orderDetail.getProduct().getId() == productId && orderDetail.getColor().getId() == colorId) {
    ...
    public String checkoutProcess(Model model,
        ...
        if (ordersaved != null && ordersaved.getId() > 0) {
            ...
            if (saveOk) {
                session.removeAttribute("order");
                String host = "http://localhost:8084/Phone_store";
                String content = "<html>\n"
                        + "   <head>\n"
                        + "      <style>\n"
                        + "         *{\n"
                ...
                try {
                    ...
                    helper.setSubject("Phone Store Order Confirmation: #" + order.getId());
